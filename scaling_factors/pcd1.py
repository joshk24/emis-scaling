...
def popen(incpath):
    ...
    bkgpath = f'{droot}/output/2023fires.v2/ASSIM/CMAQv54_cb6r5_ae7_aq.36US3.35.ASSIM.CONC.{date:%Y%j}'
    ...
    required_files = [mpath, c100path, c085path, bkgpath, tomipath, cmaqpath, lwpath] + anthro_files + fire_files + natural_files
    if not os.path.exists(incpath) or not all(os.path.exists(p) for p in required_files):
        print(f"Missing required files for {incpath}", flush=True)
        return None
    ...
    print(outf['SCALE'].mean(), outf['SCALE'].max(), outf['SCALE'].min(), flush=True)
    ...
    outf['SCALE'] = 0.9 * scale_raw + 0.1 * scale_smoothed  # Adjust weights to preserve more/less of original
    print(outf['SCALE'].mean(), outf['SCALE'].max(), outf['SCALE'].min(), flush=True)
...
dates = pd.date_range('2023-05-01', '2023-09-30')

for date in dates:
    inctmpl = f'{droot}/gsi_scripts/NO2ASSIM_v1/*/incno2.{date:%Y%m%d}??'
    ...
    hf.drop_vars(['ROW', 'COL', 'TSTEP']).to_netcdf(f'./SCALE1_FINALv2/HOURLY_SCALE_{date:%Y%m%d}.nc')
    if lstfilter:
    ...
    df.to_netcdf(f'./SCALE1_FINALv2/DAILY_SCALE_{date:%Y%m%d}.nc')
    print(f'./SCALE1_FINALv2/DAILY_SCALE_{date:%Y%m%d}.nc')
